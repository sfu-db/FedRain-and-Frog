...
class ModelManagerTest:
  def __init__(self, x_a, x_b, y, model, n_length, m_host="party_a", m_port=12345, s_host="party_b", s_port=12345):
    self.x_a = tf.Variable(x_a, name="x_a")
    self.x_b = tf.Variable(x_b, name="x_b")
    self.y = tf.Variable(y, name="y")
    self.delta = tf.Variable(
      tf.ones(shape=self.x_a.shape[0], dtype=tf.float32, name="delta"))
  ...
  def report(self, x_a_train, x_b_train, y_train, x_a_test, x_b_test, y_test):
    print("Model name:", self.model.model_name())
    print("On Training\n", classification_report(y_train.numpy(), self.model.predict(x_a_train, x_b_train).numpy()))
    print("On Testing\n", classification_report(y_test.numpy(), self.model.predict(x_a_test, x_b_test).numpy()))
    
  def get_remaining(self):
    """Get the remaining training data"""
    mask = self.delta.value() == 1
    return tf.boolean_mask(self.x_a, mask), tf.boolean_mask(self.x_b, mask), tf.boolean_mask(self.y, mask)
  ...
  def slave_send(self, data):
    ...
    while s.connect_ex(self.master_party) != 0:
      sleep(0.1)
    s.sendto(pickle.dumps(data), self.master_party)
    s.close()
    
  def master_evaluate(self):
    data_a, data_b, labels = self.get_remaining()
    return tf.squeeze(self.model(data_a, data_b)) - labels
  
  def slave_evaluate(self):
    data_a, data_b, labels = self.get_remaining()
    return tf.squeeze(self.model(data_a, data_b))
  ...
  def sgd_fn(self, data_a, data_b, labels, last_loss, opt, tol):
    with tf.GradientTape() as tape:
      current_loss = self.model.loss(data_a, data_b, labels)
    op_grad = getattr(self.model, "grad", None)
    if callable(op_grad):
      grads = op_grad(data_a, data_b, labels)
    else:
  ...
  def fit(self, max_iter=100, print_value=False, tol=1e-5, lr=0.1):
    ...
    for iteration in range(max_iter):
      data_a, data_b, labels = self.get_remaining()
      last_loss, cont = self.sgd_fn(data_a, data_b, labels, last_loss, opt, tol)
#       print(last_loss)
      if not cont:
        break
    if print_value:
  ...
  def eloss(self):
    ...
    """
    data_a, data_b, labels = self.get_remaining()
    return self.model.eloss(data_a, data_b, labels)
  
  def egrads(self, **kwargs):
    op_egrads = getattr(self.model, "egrads", None)
    if callable(op_egrads):
        data_a, data_b, labels = self.get_remaining()
        return op_egrads(data_a, data_b, labels, **kwargs)
    else:
        print("Egrads")
  ...
  def hessian(self, is_master, **kwargs):
    ...
    if callable(op_hess):
        data_a, data_b, labels = self.get_remaining()
        if is_master:
            return op_hess(data_a, data_b, labels=labels, **kwargs)
        else:
            return op_hess(data_a, data_b, **kwargs)
    else:
        print("Hessian")
        with tf.GradientTape(persistent=True) as tape:
  ...
  def egmul(self, vs):
    data_a, data_b, labels = self.get_remaining()
    egrads = self.model.egrads(data_a, data_b, labels)
