...
class MNISTJoinAggregationProcessor(Processor):
    """
    ...
    def __init__(
        ...
        assert corrupted_class in range(
            10
        ), f"corrupted_class must in 0..10, got {corrupted_class}"
        ...
        yleft = ytest[test_in_left]

        test_in_right = np.isin(ytest, partition[1])
        yright = ytest[test_in_right]
        ...
        self.set_tensor_variables(
            Xtrain=Xtrain,
            Xleft=Xtest[test_in_left, :],
            Xright=Xtest[test_in_right, :],
    ...
    def complain(self, manager: ModelManager, exact: bool = False) -> ComplaintRet:
        return complain_impl(
            self.Xleft, self.yleft, self.Xright, self.yright, manager, exact
        )
